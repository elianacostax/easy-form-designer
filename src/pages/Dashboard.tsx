...
const Dashboard = () => {
  ...
  const stats = [
    { label: "Guardadas", value: saved.length, icon: Heart },
    { label: "Vistas esta semana", value: 42, icon: Eye },
    { label: "Siguiendo", value: 7, icon: Layers },
  ];
  ...
  return (
    <div className="px-6 lg:px-16 py-12 pb-32 lg:pb-16">
      <header className="mb-16 flex items-end justify-between flex-wrap gap-6">
        <div>
          <p className="text-[11px] tracking-[0.3em] uppercase text-muted-foreground mb-3">Hola de nuevo</p>
          <h1 className="font-serif text-5xl md:text-6xl">Tu sala privada.</h1>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center font-serif text-xl">M</div>
          <div>
            <p className="text-sm font-medium">María Coleccionista</p>
            <p className="text-xs text-muted-foreground">Miembro desde 2025</p>
          </div>
        </div>
      </header>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-[11px] tracking-[0.3em] uppercase text-muted-foreground mb-3 inline-flex items-center gap-2">
              <Sparkles className="w-3.5 h-3.5" /> Recomendado para ti
            </p>
            <h2 className="font-serif text-4xl">Curado según tu gusto.</h2>
          </div>
        </div>
      ...
      <section className="mb-20">
        <h2 className="font-serif text-4xl mb-8">Artistas que sigues</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {artists.map((a) => (
      ...
      <section>
        <h2 className="font-serif text-4xl mb-8">Explorar por categoría</h2>
