...
const ArtistProfile = () => {
  ...
  if (!artist) return <div className="p-16 font-serif text-3xl">Artista no encontrado.</div>;
  ...
  return (
    <div className="pb-32 lg:pb-16 animate-fade-in">
      <div className="relative h-[55vh] min-h-[400px] overflow-hidden -mt-20 lg:mt-0">
        <img src={artist.cover} alt={`Portada de ${artist.name}`} className="w-full h-full object-cover" />
      ...
      <div className="px-6 lg:px-16 -mt-32 lg:-mt-40 relative z-10">
        <div className="flex flex-col md:flex-row gap-8 md:items-end">
          ...
          <div className="flex-1">
            <p className="text-[11px] uppercase tracking-[0.3em] text-muted-foreground mb-2">{artist.origin} · n. {artist.born}</p>
            ...
            <p className="mt-3 inline-flex items-center gap-2 text-sm text-foreground/80">
              <GraduationCap className="w-4 h-4" strokeWidth={1.5} /> {artist.school}
            </p>
          </div>
          <button className="inline-flex items-center gap-2 bg-ink text-ivory rounded-full px-6 py-3 text-sm">
            <Plus className="w-4 h-4" strokeWidth={1.5} /> Seguir
          </button>
        </div>
        ...
        <div className="mt-12 grid lg:grid-cols-3 gap-12">
          ...
          <dl className="space-y-6 border-l border-border/60 pl-8">
            {[["Obras", artist.works], ["Seguidores", artist.followers], ["Origen", artist.origin]].map(([k, v]) => (
              <div key={k as string}>
        ...
        <section className="mt-24">
          <h2 className="font-serif text-4xl mb-10">Obras seleccionadas</h2>
